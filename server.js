...
const storage=multer.diskStorage({
    ...
    filename:function(req,file,cb){
        const IMGNAME= Date.now()+'-'/*new Date().toISOString().replace(/:/g, '-')*/+file.fieldname+file.mimetype.replace("/",'.');
        cb(null,IMGNAME);
    }
});
...
app.post('/Admin',upload.single('IMAGE'),(req,res)=>{
    ...
    respo={
        ...
        Profil:".."+ req.file.path.replace(/\\/g, "/").substring("public".length),
        state:"indisponible",
    ...
    if( req.cookies["UserName"]!=null)
    {
        ...
        try{
            const account=await db.getDB().collection(Livreurs).findOne(ObjectId(livId));
                if(account!=null)
                {
                    account.Profil=(account.Profil).replace('../','../public/');
...
app.post('/Commander',(req,res)=>{
    ...
    // Getting thee distance between the client position and the delivery guy pos
    const pos2={latitude:31.6517314,longitude:-8.0631584}
    let distance=geolib.getDistance(Info.Location,pos2)
...
app.post('/Food_Edit',upload.single('Image_Update'),(req,res)=>{
    ...
    // var file=req.body.Image_Update;
    if(req.file.size!=0)
    {
        ...
        // Here is adding the new image plus adding the infos
        ...
        db.getDB().collection(Foods).findOne(ObjectId(Edited_Id),(err,doc)=>{
            if(err)
            {
                ...
                var Image_Path=doc.Image;
                    ...
                    console.log(NewFood)
                    // db.getDB().collection(Foods).UpdateOne(ObjectId(Edited_Id),NewFood,(err,result)=>{
                    //     if(err)
                    //     {
                    //         console.log(err)
                    //     }
                    //     else{
                    //         console.log('FOOD EDITED : ',result);
                    //         // alert('Food  Edited !')
                                // Deleting the image
                        //  Image_Path="public/"+Image_Path.replace('..','')
                        //     fs.unlink(Image_Path,function(err,cb)
